...
def init_db():
    ...
    """)
    # Add soil_percent column to existing tables that lack it
    try:
...
def receive_sensor_data():
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO sensor_readings (soil_moisture, soil_percent, temperature, humidity, pump_status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        (soil, soil_pct, temp, hum, pump, now),
    )
    conn.commit()
    conn.close()
